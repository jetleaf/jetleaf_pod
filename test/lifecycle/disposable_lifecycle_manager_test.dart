...
class MockDestructionAwarePodProcessor extends DestructionAwarePodProcessor {
  ...
  @override
  Future<void> processBeforeDestruction(Object pod, Class podClass, String podName) async {
    beforeCalled = true;
  }
  
  @override
  Future<void> processAfterDestruction(Object pod, Class podClass, String podName) async {
    afterCalled = true;
  }
  
  @override
  Future<bool> requiresDestruction(Object pod, Class podClass, String podName) async {
    requiresCalled = true;
    return true;
  }
}
...
void main() {
  ...
  group('DisposableLifecycleManager', () {
    ...
    test('onDestroy should call both methods when pod implements both', () async {
      final pod = TestBoth();
    ...
    test('should call processors before and after destruction', () async {
      ...
      final manager = DisposableLifecycleManager(pod, 'test', definition, [processor]);
    ...
    test('hasDestroyMethod should work for disposable pods', () {
      ...
      expect(DisposableLifecycleManager.hasDestroyMethod(pod, definition), isTrue);
    });
    ...
    test('hasApplicableProcessors should return true when processors exist', () {
      final pod = TestDisposablePod();
      final definition = RootPodDefinition(type: Class<TestDisposablePod>());
      final processor = MockDestructionAwarePodProcessor();
      
      expect(DisposableLifecycleManager.hasApplicableProcessors(pod, definition, {processor}), isTrue);
    });
    ...
    test('filterPostProcessors should filter processors', () async {
      ...
      final filtered = await DisposableLifecycleManager.filterPostProcessors({processor}, pod, definition);
