...
class TestAbstractPodFactory extends AbstractPodFactory {
  ...
  void addDefinition(String name, PodDefinition definition) => _definitions[name] = definition;
  void addProvider(String name, PodProvider provider) => _providers[name] = provider;
  ...
  @override
  bool getAllowRawInjectionEvenWhenWrapped() => false;
  ...
  group('AbstractPodFactory Tests', () {
    ...
    group('Basic Pod Operations', () {
      ...
      test('should get pod with arguments', () async {
        ...
        final args = [ArgumentValue('arg1', qualifiedName: "dart:core/string.dart.String")];
    ...
    group('Pod Definition Management', () {
      ...
      test('should get number of pod definitions', () {
        ...
        factory.addDefinition('service', RootPodDefinition(type: Class<TestService>()));
        expect(factory.getNumberOfPodDefinitions(), equals(1));
        
        factory.addDefinition('repository', RootPodDefinition(type: Class<TestRepository>()));
    ...
    group('Pod Creation', () {
      ...
      test('should create pod with arguments', () async {
        final definition = RootPodDefinition(type: Class<TestService>());
        final args = [ArgumentValue('test-arg', qualifiedName: "dart:core/string.dart.String")];
    ...
    group('Singleton Management', () {
      test('should register singleton with object', () async {
        ...
        await factory.registerSingleton('singletonService', Class<TestService>(), object: ObjectHolder<Object>(service, packageName: "test"));
      ...
      test('should register singleton with factory', () async {
        ...
        await factory.registerSingleton('factoryService', Class<TestObjectFactory>(), factory: objectFactory);
    ...
    group('Pod Processors', () {
      test('should add pod aware processor', () {
        ...
        expect(factory.getPodAwareProcessors(), contains(processor));
      });
      
      test('should get pod processor count', () {
        expect(factory.getPodAwareProcessorCount(), equals(0));
        
        factory.addPodAwareProcessor(TestPodProcessor());
        expect(factory.getPodAwareProcessorCount(), equals(1));
        
        factory.addPodAwareProcessor(TestPodProcessor());
        expect(factory.getPodAwareProcessorCount(), equals(2));
      });
      
      test('should check if has instantiation pod post processors', () {
        expect(factory.hasInstantiationPodPostProcessors(), isFalse);
        
        factory.addPodAwareProcessor(TestInstantiationProcessor());
        expect(factory.hasInstantiationPodPostProcessors(), isTrue);
      });
      
      test('should check if has destruction pod post processors', () {
        expect(factory.hasDestructionPodPostProcessors(), isFalse);
        
        factory.addPodAwareProcessor(TestDestructionProcessor());
        expect(factory.hasDestructionPodPostProcessors(), isTrue);
      });
    });
    ...
    group('Error Handling and Edge Cases', () {
      ...
      test('should handle multiple processors of same type', () {
        ...
        factory.addPodAwareProcessor(processor1);
        factory.addPodAwareProcessor(processor2);
        
        expect(factory.getPodAwareProcessorCount(), equals(2));
        expect(factory.getPodAwareProcessors(), contains(processor1));
        expect(factory.getPodAwareProcessors(), contains(processor2));
      });
    });
  });
}
...
class TestDestructionProcessor extends DestructionAwarePodProcessor {
  @override
